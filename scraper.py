...
# -*- coding: utf-8 -*-
"""
scraper.py — Supabase schema-aligned + Handle resolve + Gemini AI + Auto-discover (daily)

Fixes:
- Merge handle-only rows into existing channel_id row, then delete handle-row (avoid channels_channel_id_key duplicate)
- Dedup video_rows by video_id before upsert (avoid "ON CONFLICT ... affect row a second time")
- Upsert only (no insert fallback that breaks on duplicates)
"""
...
async def yt_playlist_items_video_ids(session: aiohttp.ClientSession, api_key: str, uploads_playlist_id: str, limit: int) -> List[str]:
    ...
    while len(out) < limit:
        params = {"part": "contentDetails", "playlistId": uploads_playlist_id, "maxResults": min(50, limit - len(out)), "key": api_key}
        if page_token:
...
async def yt_videos_details(session: aiohttp.ClientSession, api_key: str, video_ids: List[str]) -> List[Dict[str, Any]]:
    ...
    params = {"part": "snippet,statistics", "id": ",".join(video_ids[:50]), "key": api_key, "maxResults": 50}
    data = await fetch_json_get(session, url, params)
...
def list_channels(client: Client) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if cid:
            if cid not in by_cid or int(row["id"]) < int(by_cid[cid]["id"]):
                by_cid[cid] = row
        else:
            if h:
                if h not in by_handle or int(row["id"]) < int(by_handle[h]["id"]):
                    by_handle[h] = row

    return list(by_cid.values()) + list(by_handle.values())


def get_channel_row_by_channel_id(client: Client, channel_id: str) -> Optional[Dict[str, Any]]:
    try:
        r = client.table("channels").select("id,channel_id,handle").eq("channel_id", channel_id).limit(1).execute()
        row = (r.data or [None])[0]
...
def delete_channel_row_by_id(client: Client, row_id: int, dry: bool) -> None:
    if dry:
        print(f"[DRY] delete channels.id={row_id}")
        return
...
def update_channel_id_by_row_id(client: Client, row_id: int, channel_id: str, dry: bool) -> None:
    if dry:
        ...
        return
    client.table("channels").update({"channel_id": channel_id}).eq("id", int(row_id)).execute()
...
def upsert_videos(client: Client, rows: List[Dict[str, Any]], dry: bool) -> None:
    if not rows:
        return
    uniq: Dict[str, Dict[str, Any]] = {}
    for r in rows:
        vid = safe_str(r.get("video_id")).strip()
        if not vid:
            continue
        uniq[vid] = r
    ...
    if dry:
        print(f"[DRY] upsert videos (deduped): {len(rows)}")
        return
    ...
    try:
        client.table("videos").upsert(payload, on_conflict="video_id").execute()
    except Exception as e:
        msg = str(e)
        if "column" in msg and ("niche" in msg or "sentiment" in msg):
            for p in payload:
                p.pop("niche", None)
...
def insert_snapshots(client: Client, rows: List[Dict[str, Any]], dry: bool) -> None:
    if not rows:
        return
    uniq = {}
    for r in rows:
        k = (safe_str(r.get("video_id")), safe_str(r.get("captured_at")))
        uniq[k] = r
    rows = list(uniq.values())

    if dry:
        print(f"[DRY] insert snapshots (deduped): {len(rows)}")
        return
...
async def auto_discover_channels(session: aiohttp.ClientSession, cfg: Cfg, client: Client) -> int:
    ...
    for i in range(0, len(found), 50):
        ...
        for it in items:
            ...
            candidates.append({"channel_id": cid, "title": title, "handle": handle, "avatar_url": avatar, "subscribers": subs})
...
async def process_one_channel(
    ...
    async with yt_sem:
        ...
        if real_cid and (not channel_id) and row_id is not None:
            existing = get_channel_row_by_channel_id(client, real_cid)
            if existing and int(existing["id"]) != int(row_id):
                # merge handle if missing
                try:
                    if (not (existing.get("handle") or "").strip()) and handle:
                        if cfg.dry_run:
                            print(f"[DRY] merge handle {handle} -> channels.id={existing['id']}")
                        else:
                            client.table("channels").update({"handle": handle}).eq("id", int(existing["id"])).execute()
                except Exception as e:
                    print(f"[WARN] merge handle failed: {e}")

                # delete handle row
                try:
                    delete_channel_row_by_id(client, int(row_id), cfg.dry_run)
                except Exception as e:
                    print(f"[WARN] delete handle-row failed: {e}")
        ...
        upsert_channels(client, [{
            "channel_id": channel_id or real_cid,
    ...
    if not uploads or not channel_id:
        return [], []
...
async def run_async(cfg: Cfg) -> None:
    ...
    headers = {"User-Agent": "toolwatch-scraper/4.1", "Accept": "application/json"}
    async with aiohttp.ClientSession(headers=headers) as session:
        results = await asyncio.gather(
            *[process_one_channel(yt_sem, ai_sem, session, cfg, client, r) for r in rows],
        ...
        upsert_videos(client, all_videos, cfg.dry_run)
        insert_snapshots(client, all_snaps, cfg.dry_run)
